// ============================================================
//  TRIXY.LOOKS — Cloudflare Worker (All-in-One)
//  Serve HTML + KV API dengan PIN auth
...
<style>
:root {
  --pink:#FF6B9D;--purple:#A855F7;--blue:#3B82F6;--teal:#14B8A6;
  --yellow:#FCD34D;--green:#22C55E;--red:#F43F5E;--orange:#FB923C;
  --bg:#FFF5FB;--surface:#FFFFFF;--surface2:#FDF0F8;--border:#F0D6EB;
  --text:#2D1B47;--muted:#9B7BB8;--radius:18px;--radius-sm:10px;
  --grad:linear-gradient(135deg,#FF6B9D 0%,#A855F7 50%,#3B82F6 100%);
}
...
body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;
  background:radial-gradient(circle at 10% 20%,rgba(255,107,157,.08) 0%,transparent 40%),
  radial-gradient(circle at 90% 10%,rgba(168,85,247,.07) 0%,transparent 40%),
  radial-gradient(circle at 80% 80%,rgba(59,130,246,.06) 0%,transparent 40%),
  radial-gradient(circle at 20% 90%,rgba(20,184,166,.06) 0%,transparent 40%);}
...
.pin-logo{font-family:'Baloo 2',cursive;font-size:28px;font-weight:800;
  background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px}
...
.pin-btn{
  ...
  background:#fff;font-family:'Baloo 2',cursive;font-size:20px;font-weight:800;
...
/* ── LOADING ── */
#loading-overlay{
  position:fixed;inset:0;z-index:500;background:rgba(255,245,251,.9);
...
#sync-status{
  ...
  border-radius:30px;padding:8px 16px;font-size:12px;font-weight:800;
  color:var(--muted);box-shadow:0 4px 16px rgba(0,0,0,.1);
...
/* ── HEADER ── */
.header{
  background:var(--grad);padding:14px 20px;
  display:flex;align-items:center;gap:14px;
  position:sticky;top:0;z-index:100;
  box-shadow:0 4px 20px rgba(168,85,247,.25);
}
.logo-wrap{display:flex;flex-direction:column}
.logo{font-family:'Baloo 2',cursive;font-size:22px;font-weight:800;color:#fff;line-height:1}
.logo-sub{font-size:10px;color:rgba(255,255,255,.8);font-weight:700;letter-spacing:2px;text-transform:uppercase}
.header-badge{margin-left:auto;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.35);border-radius:30px;padding:4px 12px;font-size:11px;font-weight:800;color:#fff;letter-spacing:.5px}
.header-sync{
...
/* ── TABS ── */
.tabs{display:flex;gap:4px;padding:10px 16px;background:var(--surface);border-bottom:2px solid var(--border);overflow-x:auto;position:sticky;top:60px;z-index:99;box-shadow:0 2px 12px rgba(168,85,247,.08)}
...
/* ── LAYOUT ── */
.page{display:none;padding:16px;max-width:860px;margin:0 auto;position:relative;z-index:1}
...
.card-title{font-family:'Baloo 2',cursive;font-size:15px;font-weight:800;color:var(--purple);margin-bottom:14px;display:flex;align-items:center;gap:8px}
...
.stat-value{font-size:18px;font-weight:900;font-family:'JetBrains Mono',monospace}
...
.filter-bar input,.filter-bar select{padding:7px 12px;font-size:12px;flex:1;min-width:120px}
...
<div id="pin-screen">
  <div class="pin-card">
    ...
    <div class="pin-grid">
      <div class="pin-btn" onclick="pinInput('1')">1</div>
      <div class="pin-btn" onclick="pinInput('2')">2</div>
      <div class="pin-btn" onclick="pinInput('3')">3</div>
      <div class="pin-btn" onclick="pinInput('4')">4</div>
      <div class="pin-btn" onclick="pinInput('5')">5</div>
      <div class="pin-btn" onclick="pinInput('6')">6</div>
      <div class="pin-btn" onclick="pinInput('7')">7</div>
      <div class="pin-btn" onclick="pinInput('8')">8</div>
      <div class="pin-btn" onclick="pinInput('9')">9</div>
      <div class="pin-btn pin-del" onclick="pinDelete()">⌫</div>
      <div class="pin-btn" onclick="pinInput('0')">0</div>
      <div class="pin-btn pin-enter" onclick="pinSubmit()">✓</div>
    </div>
  </div>
</div>
...
<div id="app" style="display:none">
  <div class="header">
    ...
    <div class="header-sync" id="sync-btn" onclick="syncAll()" title="Refresh data">🔄</div>
    <div class="header-badge">🛍️ Online Shop</div>
  </div>
  ...
  <div class="page active" id="page-produk">
    <div class="card">
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Target Margin Keuntungan (Rp)</label><input type="number" id="p-margin" placeholder="20000" oninput="hitungHarga()"></div>
      ...
      <div id="price-result-box" class="price-result" style="display:none">
        ...
        <div class="price-breakdown">
          <div class="price-item"><div class="pr-label">Modal Total</div><div class="piv" style="color:var(--purple)" id="r-modal-total">Rp 0</div></div>
          <div class="price-item"><div class="pr-label">Admin Shopee</div><div class="piv" style="color:var(--red)" id="r-admin-rp">Rp 0</div></div>
          <div class="price-item"><div class="pr-label">Untung Bersih</div><div class="piv" style="color:var(--green)" id="r-untung">Rp 0</div></div>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table><thead><tr><th>Nama</th><th>Modal</th><th>Admin%</th><th>Margin (Rp)</th><th>Harga Jual</th><th>Stok</th><th>Aksi</th></tr></thead>
          <tbody id="tbody-produk"><tr><td colspan="7"><div class="empty"><div class="icon">📦</div><p>Belum ada produk</p></div></td></tr></tbody>
        </table>
      </div>
    </div>
  </div>
  ...
  <div class="page" id="page-jual">
    <div class="card">
      ...
      <div id="pay-shopee-box" style="display:none;background:#FFF0E0;border:2px solid var(--orange);border-radius:var(--radius-sm);padding:12px">
        <span style="font-size:13px;font-weight:800;color:var(--orange)">🛍️ Seluruh pembayaran masuk via Shopee</span>
      </div>
      ...
      <div id="pay-cash-box" style="display:none;background:#DCFCE7;border:2px solid var(--green);border-radius:var(--radius-sm);padding:12px">
        <span style="font-size:13px;font-weight:800;color:var(--green)">💵 Seluruh pembayaran tunai / transfer</span>
      </div>
      <div class="total-bar">
  ...
  <div class="page" id="page-pengeluaran">
    ...
    <div class="card">
      ...
      <div class="filter-bar">
        <input type="date" id="kel-filter-tgl" oninput="renderPengeluaran()" style="max-width:160px">
        <button class="btn btn-ghost btn-sm" onclick="document.getElementById('kel-filter-tgl').value='';renderPengeluaran()">Semua</button>
      </div>
      <div class="table-wrap">
  ...
  <div class="page" id="page-transaksi">
    ...
    <div class="card">
      ...
      <div class="filter-bar">
        <input type="date" id="trx-filter-tgl" oninput="renderTransaksi()" style="max-width:160px">
  ...
  <div class="page" id="page-analitik">
    <div class="filter-bar" style="margin-bottom:14px">
      <input type="date" id="an-filter-tgl" oninput="renderAnalitik()" style="max-width:160px">
    ...
    <div class="stats-row">
      <div class="stat-card pink"><div class="stat-label">Penjualan Hari Ini</div><div class="stat-value pink mono" id="an-hari-jual">Rp 0</div><div class="stat-sub" id="an-hari-trx">0 trx</div></div>
      <div class="stat-card orange"><div class="stat-label">Pengeluaran Hari Ini</div><div class="stat-value orange mono" id="an-hari-kel">Rp 0</div></div>
      <div class="stat-card green"><div class="stat-label">Laba Hari Ini</div><div class="stat-value green mono" id="an-hari-laba">Rp 0</div></div>
      <div class="stat-card purple"><div class="stat-label">Penjualan Bulan Ini</div><div class="stat-value purple mono" id="an-bulan-jual">Rp 0</div><div class="stat-sub" id="an-bulan-trx">0 trx</div></div>
      <div class="stat-card teal"><div class="stat-label">Pengeluaran Bulan Ini</div><div class="stat-value teal mono" id="an-bulan-kel">Rp 0</div></div>
      <div class="stat-card blue"><div class="stat-label">Laba Bulan Ini</div><div class="stat-value blue mono" id="an-bulan-laba">Rp 0</div></div>
    </div>
    <div class="card">
...
<script>
// ════════════════════════════════════════════
//  CONFIG — ganti BASE_URL sesuai Worker kamu
// ════════════════════════════════════════════
const BASE_URL = "https://trixylooks.rinaldy-fummi.workers.dev";
...
let selectedKatEl = null;
let selectedKat = '';
let trxFilterMode = 'all';
let anFilterMode = 'all';
...
async function pinSubmit() {
  ...
  try {
    const res = await fetch(BASE_URL + "/api/verify-pin", { headers: { "X-PIN": PIN } });
    if (res.ok) {
    ...
    hideLoading();
    showPinError("Gagal terhubung ke server");
    PIN = ""; pinBuffer = ""; updatePinDots();
  }
}
...
async function apiGet(key) {
  const res = await fetch(BASE_URL + "/api/" + key, { headers: { "X-PIN": PIN } });
...
async function loadAll() {
  ...
  try {
    [produk, transaksiJual, transaksiKel] = await Promise.all([
      apiGet("produk"), apiGet("jual"), apiGet("pengeluaran")
    ]);
    renderProduk(); renderPengeluaran(); renderTransaksi(); 
...
async function saveData(key) {
  setSyncBar(true);
  try {
    const data = key === "produk" ? produk : key === "jual" ? transaksiJual : transaksiKel;
    await apiSet(key, data);
    setSyncStatus("☁️ Tersimpan");
  } catch (e) {
    setSyncStatus("⚠️ Gagal menyimpan — cek koneksi");
    toast("Gagal simpan! Cek koneksi internet", "red");
  }
  setSyncBar(false);
}
...
let syncBarTimer;
function setSyncBar(on) {
  const el = document.getElementById('sync-bar');
  el.style.transform = on ? 'scaleX(0.7)' : 'scaleX(0)';
}
...
function setSyncStatus(msg) {
  ...
  syncStatusTimer = setTimeout(() => el.classList.remove('show'), 2500);
}
...
function toast(msg, type = 'pink') {
  const t = document.getElementById('toast');
  t.textContent = msg;
  const cols = { pink: 'var(--pink)', green: 'var(--green)', red: 'var(--red)', orange: 'var(--orange)', purple: 'var(--purple)' };
  t.style.background = cols[type] || cols.pink;
...
function hitungHarga() {
  ...
  const divisor = 1 - (admin / 100);
  if (divisor <= 0) return;
  const hargaJual = Math.ceil((modalTotal + marginRp) / divisor / 100) * 100;
  const adminRp = hargaJual * (admin / 100);
  const untung = hargaJual - modalTotal - adminRp;
  const marginPct = hargaJual > 0 ? (untung / hargaJual * 100).toFixed(1) : 0;
  document.getElementById('price-result-box').style.display = 'flex';
  document.getElementById('r-harga-jual').textContent = rp(hargaJual);
  document.getElementById('r-modal-total').textContent = rp(modalTotal);
  document.getElementById('r-admin-rp').textContent = rp(adminRp);
  document.getElementById('r-untung').textContent = rp(untung);
  document.getElementById('r-margin-pct').textContent = 'Margin aktual: ' + marginPct + '%';
}

function calcProduct(modal, ongkir, biaya, admin, marginRp) {
  const modalTotal = parseFloat(modal || 0) + parseFloat(ongkir || 0) + parseFloat(biaya || 0);
  const divisor = 1 - (parseFloat(admin || 2) / 100);
  if (divisor <= 0) return { hargaJual: 0, modalTotal, adminRp: 0, untung: 0 };
  const hargaJual = Math.ceil((modalTotal + parseFloat(marginRp || 0)) / divisor / 100) * 100;
  const adminRp = hargaJual * (parseFloat(admin || 2) / 100);
  const untung = hargaJual - modalTotal - adminRp;
  return { hargaJual, modalTotal, adminRp, untung };
}
...
async function tambahProduk() {
  ...
  const { hargaJual, modalTotal } = calcProduct(modal, ongkir, biaya, admin, marginRp);
  if (editProdukId) {
    const idx = produk.findIndex(p => p.id === editProdukId);
    if (idx > -1) produk[idx] = { ...produk[idx], nama, modal, ongkir, biaya, admin, marginRp, hargaJual, modalTotal, stok };
    editProdukId = null;
  } else {
    produk.push({ id: genId(), nama, modal, ongkir, biaya, admin, marginRp, hargaJual, modalTotal, stok });
  }
  renderProduk(); resetFormProduk();
  await saveData("produk");
  toast('Produk disimpan! 📦', 'pink');
}

function resetFormProduk() {
  ['p-nama', 'p-modal', 'p-ongkir', 'p-biaya', 'p-stok'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('p-admin').value = '2';
  document.getElementById('p-margin').value = '';
...
function editProduk(id) {
  ...
  document.getElementById('p-ongkir').value = p.ongkir;
  document.getElementById('p-biaya').value = p.biaya;
  document.getElementById('p-admin').value = p.admin;
  document.getElementById('p-margin').value = p.marginRp;
  document.getElementById('p-stok').value = p.stok;
  hitungHarga(); window.scrollTo({ top: 0, behavior: 'smooth' });
  toast('Mode edit aktif ✏️', 'purple');
}

async function hapusProduk(id) {
  if (!confirm('Hapus produk ini?')) return;
  produk = produk.filter(p => p.id !== id);
  renderProduk(); await saveData("produk");
  toast('Produk dihapus', 'red');
}

function renderProduk() {
  document.getElementById('jumlah-produk').textContent = produk.length + ' produk';
  const tbody = document.getElementById('tbody-produk');
  if (!produk.length) { tbody.innerHTML = \`<tr><td colspan="7"><div class="empty"><div class="icon">📦</div><p>Belum ada produk</p></div></td></tr>\`; return; }
  tbody.innerHTML = produk.map(p => \`
    <tr>
      <td><b>\${p.nama}</b></td>
      <td class="mono">\${rp(p.modal)}</td>
      ...
      <td><span class="badge \${p.stok > 5 ? 'badge-teal' : 'badge-red'}">\${p.stok}</span></td>
...
function initJual() {
  ...
  if (!sellRows.length) tambahBarisProduk();
  else renderSellRows();
  setPayMode('shopee');
}
...
function renderSellRows() {
  ...
  c.innerHTML = sellRows.map(row => {
    ...
    const sub = sel ? rp(sel.hargaJual * row.qty) : '—';
    return \`<div class="sell-row">
      <select class="prod-name" onchange="updateSellRow('\${row.id}','produkId',this.value)">
        <option value="">-- Pilih Produk --</option>\${opts}
      </select>
      <input type="number" class="qty-input" value="\${row.qty}" min="1" onchange="updateSellRow('\${row.id}','qty',this.value)">
      <div class="sub-price">\${sub}</div>
...
function updateSellRow(id, field, value) {
  const row = sellRows.find(r => r.id === id); if (!row) return;
  if (field === 'qty') row.qty = Math.max(1, parseInt(value) || 1);
...
function getTotalJual() {
  ...
  sellRows.forEach(row => {
    ...
    if (p) { total += p.hargaJual * row.qty; laba += (p.hargaJual - p.modalTotal - (p.hargaJual * p.admin / 100)) * row.qty; items += row.qty; }
  });
  return { total, laba, items };
}

function updateTotalJual() {
  const { total, laba, items } = getTotalJual();
  document.getElementById('j-total').textContent = rp(total);
  document.getElementById('j-items-summary').textContent = items + ' item dipilih';
  document.getElementById('j-profit-preview').textContent = 'Estimasi laba: ' + rp(laba);
  if (payMode === 'split') syncSplitCash();
}

function setPayMode(mode) {
  payMode = mode;
  ['shopee', 'split', 'cash'].forEach(m => {
    document.getElementById('pay-' + m + '-box').style.display = 'none';
    document.getElementById('pb-' + m).className = 'pay-btn';
  });
...
function syncSplitCash() {
  const { total } = getTotalJual();
  const s = parseFloat(document.getElementById('split-shopee').value || 0);
  const warn = document.getElementById('split-warning');
  if (s > total) { warn.style.display = 'block'; document.getElementById('split-cash').value = ''; }
  else { warn.style.display = 'none'; document.getElementById('split-cash').value = Math.round(total - s) || ''; }
}
...
async function simpanTransaksi() {
  ...
  sellRows.forEach(row => {
    ...
    const labaItem = (p.hargaJual - p.modalTotal - (p.hargaJual * p.admin / 100)) * row.qty;
    total += sub; laba += labaItem;
    items.push({ produkId: p.id, nama: p.nama, qty: row.qty, hargaJual: p.hargaJual, subtotal: sub, laba: labaItem });
  });
  if (!items.length) { toast('Pilih produk dulu!', 'red'); return; }
  let payDetail = {};
  if (payMode === 'shopee') payDetail = { mode: 'shopee', shopee: total, cash: 0 };
  else if (payMode === 'cash') payDetail = { mode: 'cash', shopee: 0, cash: total };
  else {
    const s = parseFloat(document.getElementById('split-shopee').value || 0);
    ...
    payDetail = { mode: 'split', shopee: s, cash: c };
  }
  items.forEach(item => { const p = produk.find(x => x.id === item.produkId); if (p) p.stok = Math.max(0, p.stok - item.qty); });
  transaksiJual.unshift({ id: genId(), tanggal, order: order || '-', items, total, laba, pay: payDetail });
  sellRows = []; initJual();
  await Promise.all([saveData("jual"), saveData("produk")]);
  renderProduk();
  toast('Transaksi tersimpan! 🎉', 'green');
}
...
function pilihKat(el, kat) {
  if (selectedKatEl) selectedKatEl.classList.remove('active');
  el.classList.add('active'); selectedKatEl = el; selectedKat = kat;
  document.getElementById('k-kategori').value = kat;
}

function resetFormKel() {
  ['k-jumlah', 'k-ket', 'k-catatan'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('k-tanggal').value = today();
  if (selectedKatEl) { selectedKatEl.classList.remove('active'); selectedKatEl = null; }
  selectedKat = ''; document.getElementById('k-kategori').value = '';
}

async function simpanPengeluaran() {
  const tanggal = document.getElementById('k-tanggal').value;
  const jumlah = parseFloat(document.getElementById('k-jumlah').value);
  ...
  transaksiKel.unshift({ id: genId(), tanggal, jumlah, ket, kat, catatan });
  renderPengeluaran(); resetFormKel();
  await saveData("pengeluaran");
  toast('Pengeluaran dicatat! 💸', 'orange');
}

async function hapusPengeluaran(id) {
  if (!confirm('Hapus pengeluaran ini?')) return;
  transaksiKel = transaksiKel.filter(k => k.id !== id);
  renderPengeluaran(); await saveData("pengeluaran");
  toast('Dihapus', 'red');
}
...
function renderPengeluaran() {
  const ft = document.getElementById('kel-filter-tgl')?.value;
  let data = transaksiKel;
  if (ft) data = data.filter(k => k.tanggal === ft);
  ...
  if (!data.length) { tbody.innerHTML = \`<tr><td colspan="5"><div class="empty"><div class="icon">💸</div><p>Belum ada pengeluaran</p></div></td></tr>\`; return; }
  tbody.innerHTML = data.map(k => \`
    <tr>
      <td>\${k.tanggal}</td>
      <td><span class="badge \${katStyle[k.kat] || 'badge-purple'}">\${k.kat}</span></td>
      <td>\${k.ket}\${k.catatan ? '<br><span style="font-size:11px;color:var(--muted)">' + k.catatan + '</span>' : ''}</td>
      <td class="mono fw9" style="color:var(--red)">\${rp(k.jumlah)}</td>
      <td><button class="btn btn-danger" onclick="hapusPengeluaran('\${k.id}')">🗑</button></td>
    </tr>\`).join('');
}
...
//  TRANSAKSI
// ════════════════════════════════════════════
function setTrxFilter(mode, el) {
  trxFilterMode = mode;
  document.querySelectorAll('#page-transaksi .filter-chip').forEach(x => x.classList.remove('active'));
  el.classList.add('active'); renderTransaksi();
}
function clearTrxFilter() {
  document.getElementById('trx-filter-tgl').value = ''; trxFilterMode = 'all';
  document.querySelectorAll('#page-transaksi .filter-chip').forEach((x, i) => x.classList.toggle('active', i === 0));
  renderTransaksi();
}
async function hapusTrx(id, type) {
  if (!confirm('Hapus transaksi ini?')) return;
  if (type === 'jual') transaksiJual = transaksiJual.filter(t => t.id !== id);
  else transaksiKel = transaksiKel.filter(t => t.id !== id);
  renderTransaksi();
  await saveData(type === 'jual' ? 'jual' : 'pengeluaran');
  toast('Dihapus', 'red');
}

function renderTransaksi() {
  const ft = document.getElementById('trx-filter-tgl')?.value;
  let all = [];
  ...
  if (ft) all = all.filter(x => x.tanggal === ft);

  const totalPemasukan = transaksiJual.reduce((s, t) => s + t.total, 0);
  const totalKel = transaksiKel.reduce((s, t) => s + t.jumlah, 0);
  const totalLaba = transaksiJual.reduce((s, t) => s + t.laba, 0) - totalKel;
  document.getElementById('st-pemasukan').textContent = rp(totalPemasukan);
  document.getElementById('st-pengeluaran').textContent = rp(totalKel);
  const labaEl = document.getElementById('st-laba-bersih');
  labaEl.textContent = rp(totalLaba);
  labaEl.className = 'stat-value mono ' + (totalLaba >= 0 ? 'green' : 'red');

  const tbody = document.getElementById('tbody-transaksi');
  if (!all.length) { tbody.innerHTML = \`<tr><td colspan="7"><div class="empty"><div class="icon">📋</div><p>Tidak ada transaksi</p></div></td></tr>\`; return; }
  tbody.innerHTML = all.map(x => {
    if (x.type === 'jual') {
      const itemList = x.items.map(i => i.nama + ' x' + i.qty).join(', ');
      const bayar = x.pay.mode === 'shopee' ? '<span class="badge badge-orange">Shopee</span>'
        : x.pay.mode === 'cash' ? '<span class="badge badge-green">Tunai</span>'
          : \`<span class="badge badge-blue">Split</span><br><span style="font-size:10px;color:var(--muted)">\${rp(x.pay.shopee)}+\${rp(x.pay.cash)}</span>\`;
      return \`<tr>
        <td>\${x.tanggal}</td><td><span class="badge badge-pink">💰 Pemasukan</span></td>
        <td style="max-width:160px;font-size:12px">\${x.order !== '-' ? '<b>' + x.order + '</b><br>' : ''}<span style="color:var(--muted)">\${itemList}</span></td>
        <td class="mono fw9" style="color:var(--pink)">\${rp(x.total)}</td>
        <td class="mono" style="color:var(--green)">\${rp(x.laba)}</td>
        <td>\${bayar}</td>
        <td><button class="btn btn-danger" onclick="hapusTrx('\${x.id}','jual')">🗑</button></td>
      </tr>\`;
    } else {
      return \`<tr>
        <td>\${x.tanggal}</td><td><span class="badge badge-red">💸 Pengeluaran</span></td>
...
//  ANALITIK
// ════════════════════════════════════════════
function setAnFilter(mode, el) {
  anFilterMode = mode;
  document.querySelectorAll('#page-analitik .filter-chip').forEach(x => x.classList.remove('active'));
  el.classList.add('active'); renderAnalitik();
}
function clearAnFilter() {
  document.getElementById('an-filter-tgl').value = ''; anFilterMode = 'all';
  document.querySelectorAll('#page-analitik .filter-chip').forEach((x, i) => x.classList.toggle('active', i === 0));
  renderAnalitik();
}

function renderAnalitik() {
  const ft = document.getElementById('an-filter-tgl')?.value;
  const td = ft || today(); const tm = ft ? ft.slice(0, 7) : thisMonth();
  const showJual = anFilterMode !== 'pengeluaran';
  const showKel = anFilterMode !== 'pemasukan';

  const jualToday = transaksiJual.filter(t => t.tanggal === td);
  const kelToday = transaksiKel.filter(k => k.tanggal === td);
  const jualMonth = transaksiJual.filter(t => t.tanggal.startsWith(tm));
  const kelMonth = transaksiKel.filter(k => k.tanggal.startsWith(tm));

  document.getElementById('an-hari-jual').textContent = rp(showJual ? jualToday.reduce((s, t) => s + t.total, 0) : 0);
  document.getElementById('an-hari-kel').textContent = rp(showKel ? kelToday.reduce((s, k) => s + k.jumlah, 0) : 0);
  document.getElementById('an-hari-laba').textContent = rp(jualToday.reduce((s, t) => s + t.laba, 0) - kelToday.reduce((s, k) => s + k.jumlah, 0));
  document.getElementById('an-hari-trx').textContent = jualToday.length + ' trx';
  document.getElementById('an-bulan-jual').textContent = rp(showJual ? jualMonth.reduce((s, t) => s + t.total, 0) : 0);
  document.getElementById('an-bulan-kel').textContent = rp(showKel ? kelMonth.reduce((s, k) => s + k.jumlah, 0) : 0);
  document.getElementById('an-bulan-laba').textContent = rp(jualMonth.reduce((s, t) => s + t.laba, 0) - kelMonth.reduce((s, k) => s + k.jumlah, 0));
  document.getElementById('an-bulan-trx').textContent = jualMonth.length + ' trx';

  document.getElementById('pay-shopee-t').textContent = rp(transaksiJual.filter(t => t.pay.mode === 'shopee').reduce((s, t) => s + t.total, 0));
  document.getElementById('pay-split-t').textContent = rp(transaksiJual.filter(t => t.pay.mode === 'split').reduce((s, t) => s + t.total, 0));
  document.getElementById('pay-cash-t').textContent = rp(transaksiJual.filter(t => t.pay.mode === 'cash').reduce((s, t) => s + t.total, 0));
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({
      lbl,
      jual: showJual ? transaksiJual.filter(t => t.tanggal === ds).reduce((s, t) => s + t.total, 0) : 0,
      kel: showKel ? transaksiKel.filter(k => k.tanggal === ds).reduce((s, k) => s + k.jumlah, 0) : 0
    });
  }
  drawChart(days);

  const pc = {};
  transaksiJual.forEach(t => t.items.forEach(item => {
    if (!pc[item.nama]) pc[item.nama] = { qty: 0, rev: 0 };
    pc[item.nama].qty += item.qty; pc[item.nama].rev += item.subtotal;
  }));
  ...
  else topEl.innerHTML = sorted.map(([nama, d], i) => \`
    <div style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;margin-bottom:5px">
        <span style="font-size:13px;font-weight:800">\${i + 1}. \${nama}</span>
        <span class="mono" style="font-size:12px;color:var(--purple)">\${d.qty} terjual · \${rp(d.rev)}</span>
      </div>
      <div class="prog-bar"><div class="prog-fill" style="width:\${Math.round(d.qty / maxQ * 100)}%;background:linear-gradient(90deg,var(--pink),var(--purple))"></div></div>
    </div>\`).join('');

  const kats = ['Modal Stok', 'Packing & Ongkir', 'Iklan Shopee', 'Gaji / Upah', 'Operasional Lain'];
  const catColors = ['var(--purple)', 'var(--orange)', 'var(--pink)', 'var(--blue)', 'var(--green)'];
  const katTotals = kats.map(k => ({ kat: k, total: transaksiKel.filter(x => x.kat === k).reduce((s, x) => s + x.jumlah, 0) }));
  const maxKat = Math.max(...katTotals.map(k => k.total), 1);
  const kelEl = document.getElementById('kel-cat-breakdown');
  if (!transaksiKel.length) kelEl.innerHTML = \`<div class="empty"><div class="icon">💸</div><p>Belum ada data</p></div>\`;
  else kelEl.innerHTML = katTotals.map((k, i) => \`
    <div style="margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;margin-bottom:4px">
        <span style="font-size:13px;font-weight:800">\${k.kat}</span>
        <span class="mono" style="font-size:12px;color:\${catColors[i]}">\${rp(k.total)}</span>
      </div>
      <div class="prog-bar"><div class="prog-fill" style="width:\${Math.round(k.total / maxKat * 100)}%;background:\${catColors[i]}"></div></div>
    </div>\`).join('');
}

function drawChart(days) {
  const canvas = document.getElementById('bar-chart');
  const W = canvas.parentElement.offsetWidth || 600; const H = 160;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  ctx.clearRect(0, 0, W, H);
  const maxVal = Math.max(...days.map(d => Math.max(d.jual, d.kel)), 1);
  const slotW = (W - 20) / days.length;
  const barW = Math.min(slotW * 0.28, 24); const gap = 3;
  const chartH = H - 36;
  days.forEach((d, i) => {
    const cx = 20 + i * slotW + slotW / 2;
    if (d.jual > 0) {
      const jH = Math.max(4, Math.round(d.jual / maxVal * chartH));
      const g = ctx.createLinearGradient(0, H - 24 - jH, 0, H - 24);
      g.addColorStop(0, '#FF6B9D'); g.addColorStop(1, 'rgba(255,107,157,0.3)');
      ctx.fillStyle = g; ctx.beginPath();
      ctx.roundRect(cx - barW - gap / 2, H - 24 - jH, barW, jH, [4, 4, 0, 0]); ctx.fill();
      ctx.fillStyle = '#FF6B9D'; ctx.font = 'bold 9px JetBrains Mono'; ctx.textAlign = 'center';
      const lbl = d.jual >= 1000000 ? (d.jual / 1000000).toFixed(1) + 'jt' : (d.jual / 1000).toFixed(0) + 'rb';
      ctx.fillText(lbl, cx - barW / 2 - gap / 2, H - 26 - jH);
    }
    if (d.kel > 0) {
      const kH = Math.max(4, Math.round(d.kel / maxVal * chartH));
      const g2 = ctx.createLinearGradient(0, H - 24 - kH, 0, H - 24);
      g2.addColorStop(0, '#FB923C'); g2.addColorStop(1, 'rgba(251,146,60,0.3)');
      ctx.fillStyle = g2; ctx.beginPath();
      ctx.roundRect(cx + gap / 2, H - 24 - kH, barW, kH, [4, 4, 0, 0]); ctx.fill();
    }
    ctx.fillStyle = '#9B7BB8'; ctx.font = 'bold 10px Nunito'; ctx.textAlign = 'center';
    ctx.fillText(d.lbl, cx, H - 6);
  });
}
...
export default {
  async fetch(request, env) {
    ...
    // ── Serve HTML untuk semua route lain ──────────────
